...
run_one() {
  ...
  while [ $attempt -le 2 ]; do
    ...
    restart_server
    if node scripts/exhaustive_industry_audit.mjs "$ind"; then
      return 0
    fi
    echo "!!! $ind attempt $attempt FAILED !!!"
    attempt=$((attempt+1))
  done
  echo "FATAL: $ind failed after 2 attempts"
  return 1
}

INDUSTRIES="${1:-nail spa fnb retail karaoke hotel production technical office}"
for ind in $INDUSTRIES; do
  run_one "$ind"
done

echo "=== EXHAUSTIVE AUDIT DONE ==="
